...
- [x] 函数定义（几种）以及 local function 定义的问题
- [x] 尾调用消除
- [x] thread
- [x] userdata
- [x] generic for 和 iterator
- [x] 表达式
- [ ] 为什么使用协程写的迭代器，工厂函数仅返回一个函数，不返回恒定状态及增量。
- [ ] 当协程退出（`return`）时，对应的`coroutine.resume()`返回什么？P83页判断协程完成的代码
- [ ] Lua 作为数据描述语言和持久化
- [ ]  高效的字符缓冲区算法
- [x] 弱引用表
- [x] proxy 模式
- [x] 字符串库技巧部分，匹配空字符串的模式为什么危险
- [ ] 调整斜体和代码块格式
- [ ] 通过 pandoc 编译 pdf
...
元表就是一个表，它和表关联，其中含有一些特定的字段（函数），这些函数就是元方法。在某些情况下，表会自动调用元方法。因此，元表和元方法决定了表的某些行为。在 Lua 中，只有 *table* 和 *userdata* 可以直接设置元表，其他类型必须通过 C 代码设置元表[^1]。新建的表，总是没有设置元表的。
...
注意：`-`不贪婪，总是匹配最短长度，可能是 0！

```lua
print(string.match('_ab12', '[_%a][_%w]-'))
--> _
print(string.match('_ab12', '[_%a][_%w]*'))
--> _ab12
```

捕获 %0 全部匹配 %n 第 n 个捕获。 （） 匹配字符串下标，()pattern()，第一个 () 是开始下表，第二个是结束后一位下标

javascript template string: gsub 表

URL encoding：gsub 函数

expand to tab :（）位置



`.-`往往匹配少（空），`.*`往往匹配多。将`-`放在开头和结尾都没有意义，它总需要 anchor 限制位置。
